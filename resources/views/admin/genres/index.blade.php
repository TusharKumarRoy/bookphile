    ...
    @if($genres->count() > 0)
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 p-6">
            @foreach($genres as $genre)
                <div class="relative rounded-lg border border-gray-300 bg-white px-6 py-5 shadow-sm hover:border-gray-400 hover:shadow-lg hover:-translate-y-1 hover:scale-[1.02] transform transition-all duration-300">
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <a href="{{ route('admin.genres.show', $genre) }}" class="hover:text-blue-600 transition-colors">
                                <h3 class="text-lg font-medium text-gray-900 hover:text-blue-600 cursor-pointer">{{ $genre->name }}</h3>
                            </a>
